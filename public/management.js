(function () {
  ...
  paginas.agenda = function () {
    ...
    var readOnly = !canManageAgencyTasks();
    var pageActions = readOnly ? '' : '<div class="page-head-actions"><button id="agenda-ai-import-button" class="btn btn-ghost ai-import-trigger" onclick="openCalendarAiImportFromAgenda()">✨ Colar cronograma com IA</button><button class="btn btn-primary" onclick="openTaskForm()">+ Nova atividade</button></div>';
    var aiBanner = readOnly ? '' : '<section class="agenda-ai-import-banner"><div class="agenda-ai-import-copy"><span class="agenda-ai-import-icon">✨</span><div><b>Organize o calendário usando seu cronograma escrito</b><p>Cole o planejamento completo. A IA separa datas, formatos, legendas e briefings e mostra uma prévia antes de criar os rascunhos.</p></div></div><button class="btn btn-primary" onclick="openCalendarAiImportFromAgenda()">Colar meu cronograma</button></section>';
    return '<div class="page-head"><div><h1 class="page-title">' + (readOnly ? 'Minha agenda' : 'Agenda de entregas') + '</h1><p class="page-desc">' + (readOnly ? 'Prazos das tarefas e conteúdos atribuídos ao seu perfil ou cadastro de Parceiro PJ' : 'Prazos reais das tarefas e conteúdos da agência') + '</p></div>' + pageActions + '</div>' + aiBanner +
